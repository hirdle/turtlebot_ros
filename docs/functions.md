...
3. нормализуем start_angle, end_angle по модулю 360 (через %)
4. проходим все лучи:
...
1. принимает три параметра: `distance_threshold` (допустимый скачок расстояния между соседними лучами), `min_cluster_size` (минимум точек в кластере) и `max_distance` (игнорирование всего дальше этого порога)
2. если нет scan_data, сразу вернуть []
3. пересчитываем угол шага лидару: `angle_increment = degrees(self.scan_data.angle_increment)`
4. собираем валидные измерения в список points как пары (index, distance), пропуская inf/NaN/<=0 и значения >= max_distance
5. если points пуст, вернуть []
6. формируем кластеры из соседних точек: идём по points, и если разрыв индексов > 5 или |curr_r - prev_r| > distance_threshold, начинаем новый кластер; предыдущий сохраняем только если его длина >= min_cluster_size
7. отдельно обрабатываем последний кластер, снова проверяя на min_cluster_size
8. заранее берём позицию и угол робота (x, y, yaw в радианах), чтобы не делать это в цикле
9. для каждого кластера выбираем представителя — точку с минимальной дистанцией; по её индексу считаем локальный угол: angle_deg = idx * angle_increment, потом 360 - abs(angle_deg + 180)
10. пересчитываем локальный угол в глобальный (добавляем yaw) и получаем координаты объекта: x_obj = x + dist*cos(global_angle), y_obj = y + dist*sin(global_angle)
11. в итоговый список кладём кортеж (x_obj, y_obj, dist, angle_deg); после обхода сортируем все объекты по dist (самые близкие первыми) и возвращаем список
